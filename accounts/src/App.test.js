...
test("Wow components", () => {
  expect(HELLO).toBe(100000)

  const utils = render(<Takada />)
  const container = utils.getByText(str => str.includes("SURE"))
  expect(container).toBeDefined()

  expect(container.textContent).toBe('Voila SURE YEAHHHHhhhhh')
})